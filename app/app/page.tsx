...
export default function HomePage() {
  ...
  return (
    <div className="space-y-8">
      <section className="decorative-gradient rounded-[1.75rem] p-6 ring-1 ring-border/60 sm:p-8">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="inline-flex items-center gap-2 rounded-full bg-card/80 px-3 py-1 text-sm font-bold text-primary shadow-sm">
              <Sun className="size-4" />
              {greeting()}, {profile.name}
            </p>
            <h1 className="mt-3 font-display text-2xl font-bold text-balance sm:text-3xl">
              How are you today?
            </h1>
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          ...
          <Link
            href="/app/toolbox"
            className="inline-flex min-h-12 items-center gap-2 rounded-full border-2 bg-card px-6 font-semibold transition-transform hover:scale-[1.02] focus-visible:ring-3 focus-visible:ring-ring/50 focus-visible:outline-none"
            style={{ borderColor: 'color-mix(in oklab, var(--teal) 45%, var(--card))' }}
          >
            <Wind className="size-5" style={{ color: 'color-mix(in oklab, var(--teal) 75%, var(--foreground))' }} />
      ...
      <section aria-label="Explore your space">
        <h2 className="mb-4 font-display text-xl font-bold">Your space</h2>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {AREAS.map((a) => (
            <Link
              key={a.href}
              href={a.href}
              className="group rounded-[1.6rem] focus-visible:ring-3 focus-visible:ring-ring/50 focus-visible:outline-none"
            >
              <Card
                className="flex h-full flex-col rounded-[1.6rem] border-2 p-5 transition-all group-hover:-translate-y-1 group-hover:shadow-lg"
                style={{
                  backgroundColor: `color-mix(in oklab, var(--${a.token}) 15%, var(--card))`,
                  borderColor: `color-mix(in oklab, var(--${a.token}) 38%, var(--card))`,
                }}
              >
                <span
                  className="sticker animate-wiggle flex size-14 items-center justify-center rounded-2xl"
                  style={{
                    backgroundColor: `color-mix(in oklab, var(--${a.token}) 58%, var(--card))`,
                  }}
                >
                  <a.icon className="size-7 text-foreground" strokeWidth={2.2} />
                </span>
                <h3 className="mt-4 font-display text-lg font-bold">{a.title}</h3>
                <p className="mt-1 text-sm leading-relaxed text-foreground/70">
                  {a.desc}
                </p>
                <span className="mt-4 inline-flex items-center gap-1 self-start rounded-full bg-card/70 px-3 py-1.5 text-sm font-bold text-primary">
